...
<section>
    <header>
        <h2 class="text-textLight font-medium text-gray-900 dark:text-gray-100">
            {{ __('Profile Information') }}
        </h2>

        <p class="mt-1 text-sm text-textLight dark:text-gray-400">
    ...
    <form method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        ...
        <div>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="text-textLight mt-2 text-gray-800 dark:text-gray-200">
        ...
        <div>
            ...
            @if($user->profile_image)
                <img src="{{ asset('storage/' . $user->profile_image) }}" class="mt-2 w-24 h-24 rounded-full"
                     alt="Profile Image">
            @endif
        </div>
        ...
        @if($user->role === 'candidate')
            <div>
                ...
                <textarea id="bio" name="bio"
                          class="mt-1 block w-full">{{ old('bio', $user->candidate->bio) }}</textarea>
            </div>
        @endif
        <br>
        @if($user->role === 'candidate' && $user->candidate->resume)
            <div>
                <p class="text-sm text-gray-500">Current Resume:
                    <a href="{{ asset('storage/' . $user->candidate->resume) }}" class="text-blue-500 underline"
