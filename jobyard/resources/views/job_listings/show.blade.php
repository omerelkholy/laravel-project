...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-gray-800 p-6 rounded-lg shadow-md">
            <p class="text-gray-300"><strong>Employer Name:</strong> {{ $jobListing->user->name }} </p>
            @foreach (['Description', 'Requirements', 'Benefits', 'Salary Range', 'Location', 'Work Type', 'Deadline', 'Company Logo', 'Status'] as $field)
                <p class="text-gray-300"><strong>{{ $field }}:</strong>
                    @if($field == 'Company Logo')
                        <img src="{{ asset('storage/' . $jobListing->{strtolower(str_replace(' ', '_', $field))}) }}"
                             alt="{{ $jobListing->title }} Logo" class="mt-2 max-w-xs rounded-md">
                    @else
            ...
            <div class="flex justify-end">
                <a href="{{ route('job_listings.index') }}"
                   class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">Back to Listings</a>
            </div>
        </div>
